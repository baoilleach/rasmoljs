...
# ----------------------------------------------------------------------
# Makefile generated from "Imake.tmpl" and </tmp/IIf.CsSMYk>
...
# operating system:  Linux 3.13.0-32-generic x86_64 [ELF] (3.13.0)
# libc:	(6.19.0)
# binutils:	(224)
...
SRCS = rasmol.c rasmol.h molecule.c molecule.h abstree.c abstree.h        cmndline.c cmndline.h command.c command.h tokens.h transfor.c        transfor.h render.c render.h x11win.c graphics.h pixutils.c        pixutils.h font.h outfile.c outfile.h scripts.c scripts.h infile.c        infile.h repres.c repres.h tokens.c

OBJS = rasmol.o molecule.o abstree.o cmndline.o command.o transfor.o        render.o repres.o x11win.o pixutils.o outfile.o scripts.o        infile.o tokens.o
...
all:: rasmol.$(MANNEWSUFFIX)

rasmol.$(MANNEWSUFFIX): rasmol.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname rasmol` && \
	   $(LN) `basename rasmol.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < rasmol.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) rasmol.$(MANNEWSUFFIX)

install.man:: rasmol.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) rasmol.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/rasmol.$(MANSUFFIX)
